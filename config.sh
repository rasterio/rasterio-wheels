...
function run_tests {
    ...
    cd /tmp/rasterio
    lldb --batch -k 'thread backtrace all' -k 'quit 1' -o r python -- -m pytest -vv tests -m "not gdalbin" -k "not test_ensure_env_decorator_sets_gdal_data_prefix and not test_tiled_dataset_blocksize_guard and not test_untiled_dataset_blocksize and not test_positional_calculation_byindex"
    rio --version
    rio env --formats
}
